...
// //             duration: proj.academic_year,
// //             applicants: proj._count?.groups || 0,
// //             postedById: proj.posted_by || proj.faculty_id // Capture who posted it
// //           }))
// //           setProjectOpportunities(mappedProjects)
// //         }
...
// //             postedById: internship.posted_by // Capture who posted it
// //           }))
// //           setInternshipOpportunities(mappedInternships)
// //         }
...
// //       // 1. Recruiter Isolation: Recruiters only see what they posted
// //       if (isRecruiter && opp.postedById !== currentUser?.id) {
// //         return false
// //       }
...
//             postedById: internship.posted_by // Capture who posted it
//           }))
//           console.log("Mapped Internships Data:", mappedInternships); // DEBUG LOG
//           setInternshipOpportunities(mappedInternships)
//         }
...
//       fetchOpportunities()
//     }
//   }, [currentUser]) // Changed dependency to currentUser to ensure it updates
...
//   const filteredOpportunities = combinedOpportunities.filter((opp) => {
//       if (!opp) return false
//       console.log("Posted By",opp.postedById);
//       console.log("Current User",currentUser);
//       // 1. Recruiter Isolation FIX: Force both to be Strings so "1" === 1 doesn't fail
//       if (isRecruiter && String(opp.postedById) !== String(currentUser?.id)) {
//         return false
//       }
...
//   // DEBUG LOG
//   console.log("Filtered Opportunities shown on screen:", filteredOpportunities);
...
export default function OpportunitiesPage() {
  ...
  useEffect(() => {
    const fetchOpportunities = async () => {
      ...
      try {
        ...
        // FIX: Dynamically build the internships URL with the query parameter!
        ...
        const fetchInternships = fetch(internshipsUrl).then(res => {
          if (!res.ok) throw new Error("Failed to fetch internships")
          return res.json()
        })
        ...
        if (Array.isArray(internshipsResult)) {
          const mappedInternships = internshipsResult.map((internship: any) => ({
            ...
            duration: internship.duration || "N/A",
            applicants: internship.applicants || 0,
            postedById: internship.postedBy?.user_id || internship.posted_by // Capture who posted it securely
          }))
          console.log("Mapped Internships Data:", mappedInternships); // DEBUG LOG
          setInternshipOpportunities(mappedInternships)
        }
  ...
  }, [currentUser, isRecruiter]) // Added isRecruiter to dependencies
  ...
  // --- UI SEARCH AND FILTER LOGIC ---
  const combinedOpportunities = [...internshipOpportunities, ...projectOpportunities]

  const filteredOpportunities = combinedOpportunities.filter((opp) => {
      if (!opp) return false

      // (We removed the frontend recruiter ID check here because the backend 
      // is now correctly sending only the recruiter's posts!)
